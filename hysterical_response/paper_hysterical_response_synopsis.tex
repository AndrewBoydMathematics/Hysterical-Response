...
The next section gives the Everett picture of where that drive comes from.
...
\section{Incoming particles and the leftover drive}
\label{sec:synopsis-incoming}
Picture many Everett alternatives (pointer branches).
Outgoing particles and environmental scattering wash out interference between those branches---that is decoherence.
Joos and Zeh already showed that this washing-out runs at a rate set by the flux of environmental particles; later work sharpened the same flux$\times$cross-section scaling.
So the forgetting rate tracks an outgoing / monitored scale $J_{\mathrm{out}}$.

Incoming particles can put information back in.
In the large-$n$ Everett toy used here, a single arrival cannot reshuffle the populations of many branches at leading order, so it does not change the diagonal of $\rho$.
Whatever it contributes therefore sits off the diagonal: a coherence / correlation kick.
That kick is the residual drive $\mathcal S$.
Its strength tracks the incoming rate $J_{\mathrm{in}}$.

\noindent\textit{Toy picture.}
Outgoing which-path bits split Everett alternatives (toward the classical diagonal).
Incoming unentangled information can be as large in magnitude, but it need not prefer any one branch: a shared $\lvert\phi\rangle$ on both branches is entanglement \emph{between} them, not a population reshuffle.
That is why the drive is off-classical and why the steady leftover tracks $J_{\mathrm{in}}/J_{\mathrm{out}}$---incoming feed over outgoing forgetting.

The open-systems literature already uses driven equations of the form $\dot{\delta\rho}=\mathcal L\delta\rho+\mathcal S$ for continuously fed, dissipative systems.
The full paper identifies that $\mathcal S$ with unentangled arrivals, proves it is off-diagonal in the large-$n$ toy, and notes that the steady leftover after decoherence then scales as $J_{\mathrm{in}}/J_{\mathrm{out}}$.

Important: $\mathcal S$ is the \emph{drive}, not the leftover itself.
Under exponential stability the driven equation has a unique steady state
\begin{equation}
  \delta\rho_{\mathrm{ss}}=-\mathcal L^{-1}\mathcal S,
\end{equation}
and that steady leftover is globally exponentially stable.
It vanishes if and only if $\mathcal S=0$: a nonzero residual drive produces a unique nonzero stable steady leftover (the thing whose force pairing is $F_H$).
...
Once that leftover drive is in hand, exponential stability turns the unique stable steady leftover into a Laplace integral $\delta\rho_{\mathrm{ss}}=-\mathcal L^{-1}\mathcal S$.
...
\end{equation}
so $\delta\rho_{\mathrm{ss}}=-\mathcal L^{-1}\mathcal S$ and $F_H=F_{\mathrm{resp}}=-\Tr[F\mathcal L^{-1}\mathcal S]$.
Nonzero $\mathcal S$ yields a unique nonzero exponentially stable steady leftover.
\end{theorem}
...
\begin{corollary}[Gravitational positivity at a stable saturated node]
If the closed-loop gravitational order parameter sits in the supercritical pitchfork (or equivalent saturated closed-loop) end-state, there are stable fixed points with $Y\neq0$, typically a positive node and its negative twin.
Under the usual positive-channel / positive-alignment hypotheses, leftover force on the positive branch is strictly positive: $F_H>0$ at that stable node (and $|F_H|$ is bounded away from zero).
This does \emph{not} prove that ordinary gravity ever goes critical or gets seeded into that regime---only what the saturated mathematics packages once it is there.
\end{corollary}
...
Galactic modelling continues that same leftover into a spatial halo without inventing a messenger particle.
...
In the supercritical saturated end-state the odd pitchfork supplies stable nonzero $\pm$ nodes; positive-channel alignment then makes leftover force strictly positive at the positive node.
\end{theorem}
...
Incoming particles can replenish that leftover as an off-classical drive in a large-$n$ Everett picture: outgoing bits split branches, while shared incoming information refills entanglement between them rather than preferring one population; the steady size tracks incoming feed over outgoing forgetting.
...
Push the feedback too hard and the quiet point can go unstable: dilute ``soup'' behaves more quantumly and can tip, while dense ``meatballs'' behave more classically and tend to damp the leftover locally; once saturated in the supercritical regime, a stable positive node carries strictly positive leftover force under positive-channel alignment (whether gravity actually enters that regime is deferred).
