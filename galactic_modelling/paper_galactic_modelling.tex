...
If residual-response production is the ballistic packaging of the response-theory paper's unentangled-incoming drive, then $Q$ is proportional to the total incoming rate $J_{\mathrm{in}}$ and hence to the baryonic accretion rate that supplies that channel (up to a per-arrival conversion factor).
...
\begin{remark}[Lean status]
\label{rem:ir-shell-lean}
Lean records Assumption~\ref{ass:ir-ballistic} as \texttt{IRBallisticRealization} and packages the on-shell production law as \texttt{prop\_ir\_shell}.
Reduced abstraction: the Dirac radial measure is carried by the TeX single-scale hypothesis; Lean certifies the angular-density factorization on $|u|=v$, not a measure-theoretic Dirac construction.
\end{remark}
...
Microscopic determination of $C$ and $A$ is not attempted here; a companion zero-point derivation identifies $a_0$ with the cosmological-horizon memory scale.
...
\textbf{Accretion $\to Q$ (response-theory link)} & $Q=\alpha J_{\mathrm{in}}^{\mathrm{tot}}=\alpha\dot N_{\mathrm{acc}}$ when accretion supplies the unentangled incoming channel (Prop.~\ref{prop:accretion-Q}); not the unit identity $Q=\dot\Mb$.\\
...
\textbf{Not claimed here} & New force carriers; microscopic values of $C,A,\alpha$; absolute BTFR zero point $a_0$; Galactic numerical fits; proof of the formation requirement from the spatial model.\\
\bottomrule
\end{tabular}
\end{center}
...
Proposition~\ref{prop:accretion-Q} identifies $Q$ with accretion through the response-theory paper's $J_{\mathrm{in}}$ (up to conversion $\alpha$); the formation/retention requirement $\boxed{Q\gtrsim O(\sqrt{\Mb})}$ remains a conjecture outside the spatial response model.
