...
Mature systems on the capacity--filling attractor obey the BTFR exponent $V_Q^{4}\propto\Mb$; the absolute scale $V_Q^{4}=Ga_0\Mb$ with $a_0=H_0c/2\pi$ is the zero-point theorem of~\cite{boyd2026galactic}, used numerically below at the standard BTFR calibration~\cite{mcgaugh2012,lelli2016}.
...
\label{sec:solar}
Paper~2 derives the BTFR \emph{exponent} $v_{\mathrm{f}}^{4}\propto\Mb$ from the capacity attractor and the absolute zero point
\begin{equation}
  a_0=\frac{H_0c}{2\pi}
\end{equation}
from cosmological-horizon memory of the residual channel~\cite{boyd2026galactic}.
Here we adopt that derived scale as the normalization input,
\begin{equation}
V_Q^4=Ga_0\Mb,
\end{equation}
with the numerical value $a_0=1.20\times10^{-10}\,\mathrm{m\,s^{-2}}$ matching the standard BTFR calibration~\cite{mcgaugh2012,lelli2016} (equivalently $H_0c/2\pi$ for $H_0\simeq70\,\mathrm{km\,s^{-1}\,Mpc^{-1}}$).
...
$V_Q^4=Ga_0\Mb$ with $a_0=H_0c/2\pi$ & Inherited zero-point theorem from~\cite{boyd2026galactic}; numerical $a_0$ from BTFR/$H_0$ calibration.\\
